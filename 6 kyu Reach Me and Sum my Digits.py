...
def sum_dig_nth_term(init_val, pattern_l, nth_term):
    # Iterate through the sequence to calculate terms leading up to nth_term
    for i in range(nth_term - 1):
        # Update init_val by adding the pattern value at the current index
        # The modulo operator ensures the pattern list is cycled through
        init_val += pattern_l[i % len(pattern_l)]
    
    # Calculate the sum of digits for the final term (nth_term)
    return sum(int(digit) for digit in str(init_val))
